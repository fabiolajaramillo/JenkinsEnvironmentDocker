          ...
          stage('Push Image') {
             steps {
                 withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'docker_nexus', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
                     ...
                     sh "docker tag microservicio:latest 192.168.0.11:8083/repository/docker-private/microservicio:latest"
                     sh "docker push 192.168.0.11:8083/repository/docker-private/microservicio:latest"
                 }
             }
         }
         stage('Liquibase') {
